    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while ((input = Console.ReadLine()) != "course start")
            {
                ...
                courses = SoftUniCoursePlanning(courses, command);
            }
        ...
        static List<string> SoftUniCoursePlanning(List<string> list, string[] command)
        {
            switch (command[0])
            {
                case "Add": list = Add(list, command); 
                    break;
                case "Insert": list = Insert(list, command); 
                    break;
                case "Remove": list = Remove(list, command); 
                    break;
                case "Swap": list = Swap(list, command); 
                    break;
                case "Exercise": list = Exercise(list, command); 
                    break;
            }
